...
func main() {
	for {
		weight, tall, age, sexWeight, sex := userName()
		fatRate := userInfo(weight, tall, age, sexWeight)
...
func userInfo(weight float64, tall float64, age int, sexWeight int) float64 {
...
func userName() (float64, float64, int, int, string) {
	var name string
	fmt.Print("姓名：")
	fmt.Scanln(&name)
